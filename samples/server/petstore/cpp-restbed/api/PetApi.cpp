...
void PetApiPetResource::set_callback_PUT(
	...
	callback_PUT_ = std::move(callback);
}
...
void PetApiPetPetIdResource::DELETE_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	const auto request = session->get_request();
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid pet value" : result, { {"Connection", "close"} });
				return;
			}
...
void PetApiPetFindByStatusResource::GET_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	const auto request = session->get_request();
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid status value" : result, { {"Connection", "close"} });
				return;
			}
...
void PetApiPetFindByTagsResource::GET_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	const auto request = session->get_request();
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid tag value" : result, { {"Connection", "close"} });
				return;
			}
