...
void UserApiUserResource::set_callback_POST(
...
void UserApiUserResource::POST_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	session->fetch(content_length,
		[ this ]( const std::shared_ptr<restbed::Session> session, const restbed::Bytes & body )
		{
			...
			if (status_code == 0) {
				session->close(0, result.empty() ? "successful operation" : result, { {"Connection", "close"} });
				return;
			}
...
void UserApiUserCreateWithArrayResource::set_callback_POST(
	std::function<std::pair<int, std::string>(
...
void UserApiUserCreateWithListResource::set_callback_POST(
	std::function<std::pair<int, std::string>(
		std::vector<std::shared_ptr<User>> const &
	)> callback) {
	callback_POST_ = std::move(callback);
...
void UserApiUserUsernameResource::set_callback_DELETE(
	std::function<std::pair<int, std::string>(
		std::string const &
	)> callback) {
...
void UserApiUserUsernameResource::set_callback_GET(
	std::function<std::pair<int, std::string>(
		std::string const &
	)> callback) {
	callback_GET_ = std::move(callback);
}
void UserApiUserUsernameResource::set_callback_PUT(
	std::function<std::pair<int, std::string>(
...
void UserApiUserUsernameResource::DELETE_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	const auto request = session->get_request();
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid username supplied" : result, { {"Connection", "close"} });
				return;
			}
			if (status_code == 404) {
				session->close(404, result.empty() ? "User not found" : result, { {"Connection", "close"} });
				return;
			}
...
void UserApiUserUsernameResource::PUT_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	session->fetch(content_length,
		[ this ]( const std::shared_ptr<restbed::Session> session, const restbed::Bytes & body )
		{
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid user supplied" : result, { {"Connection", "close"} });
				return;
			}
			if (status_code == 404) {
				session->close(404, result.empty() ? "User not found" : result, { {"Connection", "close"} });
...
void UserApiUserLoginResource::set_callback_GET(
	std::function<std::pair<int, std::string>(
...
void UserApiUserLoginResource::GET_method_handler(const std::shared_ptr<restbed::Session> session) {
	...
	const auto request = session->get_request();
			...
			if (status_code == 400) {
				session->close(400, result.empty() ? "Invalid username/password supplied" : result, { {"Connection", "close"} });
				return;
			}
...
void UserApiUserLogoutResource::set_callback_GET(
	std::function<std::pair<int, std::string>(
